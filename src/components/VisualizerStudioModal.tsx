...
export const VisualizerStudioModal: React.FC<VisualizerStudioModalProps> = ({
  ...
  // Video loops count for export (allows repeating loop/pattern for social clips & full songs)
  const [videoLoops, setVideoLoops] = useState<number>(1);

  // Helper to format seconds to M:SS
  const formatSecondsToMmSs = (totalSec: number) => {
    const mins = Math.floor(totalSec / 60);
    const secs = Math.floor(totalSec % 60);
    return `${mins}:${secs.toString().padStart(2, '0')}`;
  };

  // Calculate estimated single song cycle duration for recorder automation
  const singleSongDurationSeconds = React.useMemo(() => {
    // If an imported audio file (MP3, WAV, etc.) is active, use its real duration
    if (customAudioUrl && customAudioDuration > 0) {
      return Math.max(1, Math.round(customAudioDuration));
    }
    if (!song || !song.orderList || song.orderList.length === 0) return 8;
    ...
    return Math.max(1, Math.round(totalRows * secondsPerRow));
  }, [song, customAudioUrl, customAudioDuration]);

  const estimatedSongDurationSeconds = React.useMemo(() => {
    return singleSongDurationSeconds * videoLoops;
  }, [singleSongDurationSeconds, videoLoops]);

  const formattedSingleSongDuration = React.useMemo(() => {
    return formatSecondsToMmSs(singleSongDurationSeconds);
  }, [singleSongDurationSeconds]);

  const formattedSongDuration = React.useMemo(() => {
    return formatSecondsToMmSs(estimatedSongDurationSeconds);
  ...
  // Canvas Dimensions based on Aspect Ratio and optional Pro/Free resolution override
  const getCanvasDimensions = (resOverride?: '2k' | '720p' | '480p' | null): { width: number; height: number } => {
    if (resOverride === '480p') {
  ...
  // Discreet translucent watermark badge for free exports that skipped pro tier
  const drawWatermarkBadge = (cvs: HTMLCanvasElement) => {
    const ctx = cvs.getContext('2d');
    if (!ctx) return;
    ctx.save();
    const tagText = 'SYN-Tracker Free Export';
    ctx.font = 'bold 12px "Courier New", monospace';
    const tagW = ctx.measureText(tagText).width + 22;
    const tagH = 26;
    const tagX = cvs.width - tagW - 14;
    const tagY = cvs.height - tagH - 14;
    ...
    if ((ctx as any).roundRect) {
      (ctx as any).roundRect(tagX, tagY, tagW, tagH, 5);
    } else {
    ...
    ctx.fillText(tagText, tagX + 11, tagY + tagH / 2);
    ctx.restore();
  };
  ...
  // -------------------------------------------------------------
  // UNIVERSAL REALTIME STREAM VIDEO RECORDER (MediaRecorder)
  // Fail-safe recording engine: works in all browsers, iframes, & WebCodecs fallbacks
  // -------------------------------------------------------------
  const recordWithMediaRecorder = async (
    ...
    isCancelled: () => boolean,
    renderFrameFn: (overrideAudio: any) => void
  ): Promise<void> => {
    return new Promise<void>((resolve, reject) => {
      ...
      try {
        ...
        audioCtx = new AudioContextClass({ sampleRate: audioBuffer.sampleRate });
        ...
        const videoBitrate = fps === 60 ? 6000000 : 4000000;
        ...
        mediaRecorder.onstop = () => {
          try {
            ...
            a.download = `${safeName}_${ratio.replace(':', 'x')}_${fps}fps.${fileExt}`;
            a.click();
        ...
        const loop = () => {
          ...
          const currentAudioTime = audioCtx ? audioCtx.currentTime - startAudioTime : 0;

          if (currentAudioTime >= totalDuration) {
            onProgress({ phase: 'Finalizing recording & saving file...', percent: 100 });
          ...
          const percent = Math.min(99, Math.round((currentAudioTime / totalDuration) * 100));
          const remSec = Math.max(0, Math.ceil(totalDuration - currentAudioTime));
          const remMin = Math.floor(remSec / 60);
          const remSecStr = (remSec % 60).toString().padStart(2, '0');
          onProgress({
            phase: `Recording stream (${percent}%) • Remaining ~${remMin}:${remSecStr}`,
            percent,
          });

          // Compute frequency & waveform for visualizer
          const currentSampleIndex = Math.floor(currentAudioTime * sampleRate);
          let localBassSum = 0;
          for (let i = 0; i < 64; i++) {
          ...
          renderFrameFn({
            freq: freqArr,
            wave: waveArr,
            time: currentAudioTime * 1.8,
            frame: Math.floor(currentAudioTime * fps),
            bass: smoothBass,
            dt,
          });
  ...
  // Helper to decode an imported audio file (MP3, WAV, FLAC, OGG, M4A, etc.) to AudioBuffer at 48 kHz
  const decodeAudioFileToBuffer = async (
    fileOrUrl: File | string,
    targetSampleRate: number = 48000,
    loopsCount: number = 1,
    onProgress?: (msg: string) => void
  ): Promise<AudioBuffer> => {
    onProgress?.('Decoding imported audio file (MP3)...');
    ...
    // Process & resample to target sample rate (48 kHz) and apply loopsCount
    onProgress?.(`Processing 48 kHz audio stream (${loopsCount}x loop)...`);
    const totalDuration = decoded.duration * loopsCount;
    const totalFrames = Math.max(1, Math.ceil(totalDuration * targetSampleRate));
    const numChannels = Math.max(1, Math.min(2, decoded.numberOfChannels));

    const offlineCtx = new OfflineAudioContext(numChannels, totalFrames, targetSampleRate);
    const sourceNode = offlineCtx.createBufferSource();
    sourceNode.buffer = decoded;
    sourceNode.loop = loopsCount > 1;
  ...
  // Universal MediaRecorder stream capture fallback
  // -------------------------------------------------------------
  ...
  const handleRenderButtonClick = () => {
    if (!song && !customAudioFile && !customAudioUrl) {
      onShowToast?.('No song or audio track loaded to render.');
      return;
    }
    handleStartMasterRender(false, null, null);
  };

  const handleStartMasterRender = async (
    isWatermarked = false,
    overrideFps?: number | null,
    overrideResolution?: '2k' | '720p' | '480p' | null
  ) => {
    isWatermarkedExportRef.current = isWatermarked;
    if (!song && !customAudioFile && !customAudioUrl) {
      onShowToast?.('No song or audio track loaded to render.');
      return;
    }
    ...
    setRenderProgress({
      phase: customAudioFile || customAudioUrl ? 'Decoding imported MP3 audio...' : 'Synthesizing Studio Audio DSP...',
      frame: 0,
      totalFrames: 0,
      percent: 0,
    });
    ...
    try {
      let renderedAudioBuffer: AudioBuffer;

      if (customAudioFile || customAudioUrl) {
        renderedAudioBuffer = await decodeAudioFileToBuffer(
        ...
        // 1. Render Studio Audio to AudioBuffer with true engine FX settings (at 48 kHz standard for video)
        ...
        renderedAudioBuffer = await renderSongToAudioBuffer(
          song,
          liveFx,
          (statusMsg) => {
            setRenderProgress((prev) => ({ ...prev, phase: statusMsg }));
          ...
          { loopsCount: videoLoops, sampleRate: 48000 }
        );
      }
      ...
      const fps = overrideFps || targetFps;
      ...
      const { width: rawW, height: rawH } = getCanvasDimensions(overrideResolution);
      ...
      let offlineRenderSucceeded = false;
        ...
        try {
          ...
          if (actualFormat === 'mp4') {
            try {
              ...
              console.warn('MP4 WebCodecs encoding not supported, falling back to WebM:', err);
              actualFormat = 'webm';
              output = null;
              videoSource = null;
              audioSource = null;
          ...
          // 2. Add full rendered AudioBuffer to the audio track
          ...
          for (let f = 0; f < totalFrames; f++) {
            ...
            if (isWatermarkedExportRef.current) {
              drawWatermarkBadge(canvas);
            }
            ...
            // Yield to browser UI & Garbage Collector
          ...
          const effectiveTitle = (customAudioName || songTitle || song?.name || 'video').replace(/\.[^/.]+$/, '');
          const safeName = effectiveTitle.replace(/[^a-zA-Z0-9_-]/g, '_') || 'video';
          ...
          offlineRenderSucceeded = true;
          isMasterRenderingRef.current = false;
          setIsMasterRendering(false);
          onShowToast?.(`Video successfully rendered and saved (${actualFormat.toUpperCase()})!`);
        } catch (offlineErr: any) {
          if (cancelRenderRef.current) return;
          console.warn('WebCodecs offline encoding failed or was blocked. Falling back to universal MediaRecorder stream capture:', offlineErr);
        }
      }

      // Universal Fallback to MediaRecorder Stream Capture if offline encoding was not used or failed
      if (!offlineRenderSucceeded && !cancelRenderRef.current) {
        setRenderProgress({
          phase: 'Starting high-quality realtime stream recorder...',
          frame: 0,
          totalFrames,
          percent: 0,
        });
        ...
        await recordWithMediaRecorder(
          ...
          (p) => {
            setRenderProgress({
              phase: p.phase,
              frame: Math.round((p.percent / 100) * totalFrames),
              totalFrames,
              percent: p.percent,
            });
          },
          () => cancelRenderRef.current,
          (overrideAudio) => {
            if (renderFn) renderFn(overrideAudio);
            if (isWatermarkedExportRef.current) drawWatermarkBadge(canvas);
          }
        );

        isMasterRenderingRef.current = false;
        setIsMasterRendering(false);
        onShowToast?.(`Video successfully recorded and saved (${exportFormat.toUpperCase()})!`);
      }
    } catch (err: any) {
      if (err?.message === 'Render cancelled' || cancelRenderRef.current) {
        ...
        onShowToast?.(`Rendering notice: ${err?.message || 'Please try again.'}`);
      }
      isMasterRenderingRef.current = false;
      setIsMasterRendering(false);
    } finally {
          ...
          >
            ...
            <div className="w-full mt-3 bg-[#0e1624] border border-[#1e2d42] p-3 rounded-xl flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                {/* Loops / Video Length Selector */}
                <div className="flex items-center bg-slate-950 px-2 py-1 rounded-lg border border-slate-800 gap-1.5" title="Select how many times the song/pattern loops for video export">
                  <Clock className="w-3.5 h-3.5 text-sky-400 shrink-0" />
                  <span className="text-[11px] text-slate-400 font-medium hidden sm:inline">Length:</span>
                  <select
                    value={videoLoops}
                    onChange={(e) => setVideoLoops(Number(e.target.value))}
                    className="bg-slate-900 text-sky-300 text-[11px] font-mono font-bold rounded px-1.5 py-0.5 border border-slate-700 outline-none cursor-pointer hover:border-sky-500 transition"
                  >
                    <option value={1}>1x Loop ({formattedSingleSongDuration})</option>
                    <option value={2}>2x Loops ({formatSecondsToMmSs(singleSongDurationSeconds * 2)})</option>
                    <option value={4}>4x Loops ({formatSecondsToMmSs(singleSongDurationSeconds * 4)} - Shorts/Reels)</option>
                    <option value={8}>8x Loops ({formatSecondsToMmSs(singleSongDurationSeconds * 8)} - Social Video)</option>
                    <option value={16}>16x Loops ({formatSecondsToMmSs(singleSongDurationSeconds * 16)} - Full Track)</option>
                    <option value={32}>32x Loops ({formatSecondsToMmSs(singleSongDurationSeconds * 32)} - Extended)</option>
                  </select>
                </div>
              </div>
              ...
              <button
                ...
                className="px-4 py-2 bg-gradient-to-r from-sky-500/20 to-blue-600/20 hover:from-sky-500/30 hover:to-blue-600/30 text-sky-300 border border-sky-500/40 text-xs font-semibold rounded-xl shadow flex items-center gap-2 transition cursor-pointer disabled:opacity-50"
                ...
                <div className="bg-slate-900/60 p-3.5 rounded-xl border border-slate-800 space-y-3.5">
                  ...
                  {!bgImageDataUrl ? (
                    <div className="space-y-2">
                      <label className="flex flex-col items-center justify-center gap-2 p-3.5 border border-dashed border-slate-700 hover:border-sky-500/60 rounded-xl bg-slate-950/50 hover:bg-slate-900/60 transition cursor-pointer group">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-3 gap-1.5">
                          {[
                            ...
                            { id: 'bounce', label: 'Chiptune Bounce' },
            ...
            {activeTab === 'branding' && (
              <div className="space-y-4">
                ...
                <div className="bg-slate-900/60 p-3 rounded-xl border border-slate-800 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    {logoDataUrl && (
                      <button
                        ...
                        className="text-[10.5px] text-rose-400 hover:text-rose-300 flex items-center gap-1 cursor-pointer"
                      >
                  ...
                  {!logoDataUrl ? (
                    <label className="flex items-center justify-center gap-2 p-3 border border-dashed border-slate-700 hover:border-sky-500/60 rounded-xl bg-slate-950/50 hover:bg-slate-900/60 transition cursor-pointer group text-xs text-sky-400 hover:text-sky-300 font-medium">
                      <Upload className="w-4 h-4 text-sky-400" />
                    ...
                    <div className="flex items-center justify-between bg-slate-950/60 p-2 rounded-lg border border-slate-800/80">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <div className="w-10 h-8 rounded bg-slate-900 border border-slate-700 overflow-hidden flex items-center justify-center shrink-0">
                          <img
                            src={logoDataUrl}
                            alt="Logo preview"
                            className="w-full h-full object-contain"
                        ...
                        <span className="text-[11px] font-bold text-emerald-400 flex items-center gap-1">
                          <Check className="w-3 h-3" /> Logo Loaded
                        </span>
                      </div>
                      <label className="text-[10.5px] text-sky-400 hover:text-sky-300 cursor-pointer underline decoration-dotted inline-block">
                        Change Logo...
                        <input
                          type="file"
                          accept="image/*"

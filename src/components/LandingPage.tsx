...
export const LandingPage: React.FC<LandingPageProps> = ({
  ...
  // Scroll-driven Background Zoom Effect ("Juice")
  const scrollContainerRef = useRef<HTMLDivElement>(null);
  const rafIdRef = useRef<number | null>(null);

  useEffect(() => {
    // Initialize background zoom at 1.0 on mount
    document.documentElement.style.setProperty('--landing-bg-zoom', '1');

    return () => {
      if (rafIdRef.current) {
        cancelAnimationFrame(rafIdRef.current);
      }
      document.documentElement.style.removeProperty('--landing-bg-zoom');
    };
  }, []);
  ...
  const handleScroll = () => {
    ...
    rafIdRef.current = requestAnimationFrame(() => {
      ...
      document.documentElement.style.setProperty('--landing-bg-zoom', zoom.toFixed(4));
    });
  };
  ...
  return (
    ...
    >
      {/* 45-Degree Green Corner Ribbon */}
      <div 
        className="absolute top-0 right-0 w-36 h-36 sm:w-44 sm:h-44 overflow-hidden pointer-events-none z-[60] select-none"
        aria-hidden="true"
      >
        <div 
          className="absolute top-6 -right-12 sm:top-8 sm:-right-14 w-48 sm:w-56 py-1 sm:py-1.5 bg-gradient-to-r from-emerald-600 via-green-500 to-emerald-600 text-white font-mono shadow-[0_4px_12px_rgba(0,0,0,0.45)] border-y border-emerald-300/40 transform rotate-45 text-center flex flex-col items-center justify-center leading-none"
        >
          <span className="font-black text-[9px] sm:text-[10.5px] tracking-wider text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.6)]">
            COMPLETELY FREE
          </span>
          <span className="text-[6.5px] sm:text-[8px] font-extrabold tracking-widest text-emerald-100 uppercase mt-0.5 drop-shadow-[0_1px_2px_rgba(0,0,0,0.6)]">
            NO SUBSCRIPTION
          </span>
        </div>
      </div>
      ...
      >
        ...
        <div className="flex items-center gap-2 sm:gap-3 mr-12 sm:mr-16">
          <button
            onClick={onStart}
            className="inline-flex items-center h-8 px-3.5 sm:px-4 text-xs font-mono font-bold text-black bg-sky-400 hover:bg-sky-300 rounded-md transition-colors cursor-pointer shadow-sm"
          >
      ...
      >
        <main className="relative z-10 flex-1 flex flex-col items-center px-4 sm:px-10 pt-8 sm:pt-10 pb-16 max-w-6xl mx-auto w-full">
          
          {/* Hero Section */}
          <div className="flex flex-col items-center text-center max-w-3xl mb-8 sm:mb-10">
            <h1 className="text-2xl sm:text-5xl font-bold tracking-tight text-white mb-3 sm:mb-4 leading-tight">
              Authentic Retro Chiptune &amp; Multi-Track Tracker in Your Browser.
            </h1>

            <p className="text-xs sm:text-base text-slate-300 leading-relaxed max-w-2xl mb-5 sm:mb-6">
              Experience the direct sequencing workflow of classic Amiga trackers (ProTracker) combined with genuine sound chip models for C64 SID, Game Boy DMG, NES 2A03, Sega Mega Drive FM, 3D Cover Designer, and an integrated 60 FPS Music Visualizer.
            </p>
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3">
              <button
                onClick={onStart}
                className="h-9 sm:h-10 px-4 sm:px-6 rounded-xl bg-sky-400 hover:bg-sky-300 text-slate-950 font-bold text-xs font-mono flex items-center gap-1.5 sm:gap-2 transition-all shadow-lg shadow-sky-500/20 cursor-pointer active:scale-95"
              >
              ...
              {onLoadDemo && (
                <button
                  ...
                  className="h-9 sm:h-10 px-3 sm:px-4 rounded-xl bg-[#0e1624] hover:bg-[#152033] border border-indigo-500/40 hover:border-indigo-400 text-indigo-300 hover:text-white font-bold text-xs font-mono flex items-center gap-1.5 sm:gap-2 transition-all cursor-pointer shadow-md active:scale-95"
                >
                  <Play className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-indigo-400 fill-indigo-400" />
                  <span className="hidden sm:inline">Load Demo (Space Debris)</span>
              ...
              {onStartTutorial && (
                <button
                  ...
                  className="h-9 sm:h-10 px-3 sm:px-4 rounded-xl bg-[#0e1624] hover:bg-[#152033] border border-slate-700 hover:border-slate-500 text-slate-300 hover:text-white font-bold text-xs font-mono flex items-center gap-1.5 sm:gap-2 transition-all cursor-pointer shadow-md active:scale-95"
                >
                  <HelpCircle className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-sky-400" />
          ...
          <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              <a
                ...
                className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[#101827] hover:bg-[#172338] border border-sky-500/30 hover:border-sky-400/60 text-sky-400 hover:text-sky-300 transition-all cursor-pointer shadow-sm group"
                title="Bluesky: @hj-wuethrich.bsky.social"
              >
                <img
                  src="/bluesky.svg"
                  alt="Bluesky"
                ...
                />
                <span className="text-[11px] font-sans font-medium text-slate-300 group-hover:text-white">Bluesky</span>
              </a>

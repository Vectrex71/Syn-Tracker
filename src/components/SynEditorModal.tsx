...
// Piano keyboard mappings per layout for Live Pitch Auditioning
...
export const SynEditorModal: React.FC<SynEditorModalProps> = ({
  ...
  // Keyboard Audition States (Letter keys play sample in pitches)
  const [activeOctave, setActiveOctave] = useState<number>(3);
  const [keyboardLayout, setKeyboardLayout] = useState<KeyboardLayout>('AUTO');
  const [activeAuditionNote, setActiveAuditionNote] = useState<AuditionNoteInfo | null>(null);
  const activeAuditionKeyRef = useRef<string | null>(null);
  ...
  // Live Audition Note Trigger (Sample Playback at Custom Pitch via Keyboard/Piano)
  const handleAuditionNote = useCallback((midiNote: number, keyLabel = '') => {
    if (!currentBuffer) return;
    const targetBaseNote = samples[activeSlot]?.baseNote ?? 48;
    const targetFineTune = samples[activeSlot]?.finetune ?? samples[activeSlot]?.fineTune ?? 0;
    const targetVolume = samples[activeSlot]?.volume ?? 64;

    const sampleToAudition: TrackerSample = {
      ...samples[activeSlot],
      id: activeSlot,
      name: sampleName || `Sample_${activeSlot.toString().padStart(2, '0')}`,
      filename: samples[activeSlot]?.filename || `${sampleName || 'sample'}.wav`,
      buffer: currentBuffer,
      length: currentBuffer.length,
      loopEnabled: loopEnabled,
      loopType: loopEnabled ? 'forward' : 'none',
      loopStart: loopEnabled ? loopStart : 0,
      loopEnd: loopEnabled ? loopEnd : currentBuffer.length,
      loopLength: loopEnabled ? Math.max(0, loopEnd - loopStart) : 0,
      volume: targetVolume,
      panning: samples[activeSlot]?.panning ?? 0,
      finetune: targetFineTune,
      baseNote: targetBaseNote,
      sourceType: 'upload',
    };

    audioEngine.playNoteInstantly(midiNote, sampleToAudition);
    const noteStr = midiToNote(midiNote);
    const rate = getPlaybackRate(midiNote, targetBaseNote, targetFineTune);

    setActiveAuditionNote({
      midiNote,
      noteStr,
      keyLabel,
      playbackRate: rate,
    });
  }, [currentBuffer, samples, activeSlot, sampleName, loopEnabled, loopStart, loopEnd]);

  const handleStopAudition = useCallback(() => {
    audioEngine.stopPreviewNote();
    setActiveAuditionNote(null);
    activeAuditionKeyRef.current = null;
  }, []);
  ...
  // Keyboard Shortcuts & Piano Audition Handling
  useEffect(() => {
    if (!isOpen) return;
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // If user is holding Ctrl / Meta / Alt, do not trigger piano notes
      if (e.ctrlKey || e.metaKey || e.altKey) return;

      // Octave Shift Shortcuts: [ / - (down) and ] / + / = (up)
      if (e.key === '[' || e.key === '-') {
        e.preventDefault();
        e.stopPropagation();
      ...
      // Audition note pitch trigger via keyboard letters
      ...
      if (noteOffset === undefined) {
        ...
        const activeLayoutMap = PIANO_MAPS[keyboardLayout] || PIANO_MAPS.QWERTZ;
        if (activeLayoutMap.lower[char] !== undefined) {
          noteOffset = activeLayoutMap.lower[char];
        } else if (activeLayoutMap.upper[char] !== undefined) {
          noteOffset = activeLayoutMap.upper[char];
        } else {
          // Fallback check on QWERTZ / QWERTY
          if (PIANO_MAPS.QWERTZ.lower[char] !== undefined) {
            noteOffset = PIANO_MAPS.QWERTZ.lower[char];
          } else if (PIANO_MAPS.QWERTZ.upper[char] !== undefined) {
            noteOffset = PIANO_MAPS.QWERTZ.upper[char];
          }
        }
      }
      ...
      if (noteOffset !== undefined) {
        ...
        if (activeAuditionKeyRef.current === keyId && activeAuditionNote) {
          return;
        }
        activeAuditionKeyRef.current = keyId;
        const midi = 12 * (activeOctave + 1) + noteOffset;
        handleAuditionNote(midi, e.key.toUpperCase());
      }
    };
    ...
    const handleKeyUp = (e: KeyboardEvent) => {
      if (['INPUT', 'TEXTAREA', 'SELECT'].includes((e.target as HTMLElement)?.tagName)) return;
      const keyId = e.code || e.key;
      if (activeAuditionKeyRef.current && (activeAuditionKeyRef.current === keyId || activeAuditionKeyRef.current === e.key || activeAuditionKeyRef.current === e.code)) {
        handleStopAudition();
      }
    };
  ...
  return (
    ...
    >
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 flex-1 overflow-hidden min-h-0 p-3 gap-2.5">
            ...
            {/* BOTTOM-LEFT TOOLBAR: Audio Import & Wave/Mp3 Exporters */}
            <div className="pt-2.5 mt-2 border-t border-[#1c293a] flex items-center justify-between gap-2 shrink-0 bg-[#0b1018]">
              <div className="flex items-center gap-1.5">
                <button
                  onClick={() => fileInputRef.current?.click()}
                  className="aqua-gloss aqua-theme h-7 px-2.5 rounded flex items-center gap-1.5 cursor-pointer font-bold text-xs"
                ...
                <button
                  ...
                  className="aqua-gloss aqua-blue h-7 px-2.5 rounded flex items-center gap-1 cursor-pointer font-mono font-bold text-xs"
          ...
          >
            <SynEditorWaveform
              ...
              {/* LIVE KEYBOARD SAMPLE AUDITION BAR */}
              <SynEditorAuditionBar
                currentBuffer={currentBuffer}
                activeOctave={activeOctave}
                setActiveOctave={setActiveOctave}
                keyboardLayout={keyboardLayout}
                setKeyboardLayout={setKeyboardLayout}
                activeAuditionNote={activeAuditionNote}
                onPlayNote={(midi) => handleAuditionNote(midi, '')}
                onStopNote={handleStopAudition}
                baseNote={samples[activeSlot]?.baseNote ?? 48}
                finetune={samples[activeSlot]?.finetune ?? samples[activeSlot]?.fineTune ?? 0}
              />
            </motion.div>
          </div>
        </motion.div>

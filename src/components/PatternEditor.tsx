...
export const getTrackWidth = (channelsCount: number): number => {
  if (channelsCount <= 4) return 140;
  if (channelsCount <= 8) return 130;
  return 126;
};

const getTrackGridStyle = (channelsCount: number): React.CSSProperties => {
  const trackWidth = getTrackWidth(channelsCount);
  return {
    display: 'grid',
    gridTemplateColumns: `repeat(${channelsCount}, minmax(${trackWidth}px, 1fr))`,
    width: '100%',
    minWidth: `${channelsCount * trackWidth}px`,
  };
};
...
const PatternRow = React.memo<PatternRowProps>(({
  ...
  return (
    <div
      ...
      style={{ 
        width: '100%', 
        minWidth: `${channelsCount * getTrackWidth(channelsCount)}px` 
      }}
    >
      ...
      >
        {Array.from({ length: channelsCount }).map((_, channelIndex) => {
          ...
          return (
            <div
              key={channelIndex}
              className={`px-1.5 h-full flex items-center justify-between text-center gap-0.5 overflow-hidden box-border font-mono text-[11px] whitespace-nowrap select-none transition-colors shrink-0 ${
            ...
            >
              ...
              <span
                ...
                className={`cursor-pointer px-1 h-[17px] min-w-[28px] shrink-0 whitespace-nowrap inline-flex items-center justify-center rounded-[2px] font-mono font-bold tracking-tight border box-border select-none leading-none ${
                  isFocusedNote
                    ? isEditMode
              ...
              <span
                ...
                className={`cursor-pointer px-0.5 h-[17px] min-w-[18px] shrink-0 whitespace-nowrap inline-flex items-center justify-center rounded-[2px] font-mono font-bold border box-border select-none leading-none ${
                  isFocusedInst
                    ? isEditMode
              ...
              {/* 3. Volume / FX Column */}
              <div className="flex items-center gap-0.5 font-mono shrink-0 whitespace-nowrap">
                <span
                ...
                <span
                  ...
                  className={`cursor-pointer px-0.5 h-[17px] min-w-[12px] shrink-0 whitespace-nowrap inline-flex items-center justify-center rounded-[2px] font-mono font-bold border box-border select-none leading-none ${
                    isFocusedEff
                      ? isEditMode
                        ? 'bg-[#f43f5e] text-white border-[#fb7185] shadow-[0_0_6px_rgba(244,63,94,0.8)]'
                        : ''
...
export const PatternEditor: React.FC<PatternEditorProps> = ({
  ...
  useEffect(() => {
    ...
    const actualTrackWidth = scrollEl.scrollWidth > 0 
      ? scrollEl.scrollWidth / channelsCount 
      : getTrackWidth(channelsCount);
    const trackLeft = cursorChannel * actualTrackWidth;
    const trackRight = trackLeft + actualTrackWidth;
  ...
  const scrollToBank = (startChannelIndex: number) => {
    ...
    const actualTrackWidth = horizontalScrollRef.current.scrollWidth > 0
      ? horizontalScrollRef.current.scrollWidth / channelsCount
      : getTrackWidth(channelsCount);
    const targetLeft = startChannelIndex * actualTrackWidth;
    ...
    <div className="rounded-lg flex flex-col h-full min-h-0 overflow-hidden font-mono text-xs select-none glass-panel text-[#cbd5e1]">
      ...
      <div className="flex items-center justify-between gap-2 px-2.5 sm:px-3 py-1.5 border-b border-white/10 shrink-0 select-none bg-[#0e141d]/90 backdrop-blur-md overflow-x-auto no-scrollbar">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <div className="inline-flex items-center rounded overflow-hidden border border-[#1f364d] shadow-sm bg-[#080d14] shrink-0">
            <span 
              className="px-2 py-0.5 sm:px-2.5 sm:py-1 font-bold font-mono text-[11px] tracking-wider whitespace-nowrap"
              style={{
            ...
            >
              PAT {pattern.id.toString().padStart(2, '0')}
            </span>
          </div>

          {selectedTracks.length > 0 && (
            <div 
              className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded border backdrop-blur-sm text-[10px] sm:text-[11px] shrink-0"
              style={{
            ...
            >
              ...
              >
                {selectedTracks.length === 1 ? '1 Trk' : `${selectedTracks.length} Trks`}
              </span>
              <button
                type="button"
        ...
        <div className="flex items-center gap-1.5 sm:gap-2.5 px-1.5 sm:px-2.5 py-0.5 rounded-lg bg-[#070b10]/80 backdrop-blur-sm border border-[#1a2536] font-mono shadow-inner shrink-0">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-0.5 bg-[#0e1520]/90 border border-[#1c2738] px-1 py-0.5 rounded">
              <button
                ...
                className="px-0.5 sm:px-1 font-bold text-xs text-[#94a3b8] hover:text-white transition-colors cursor-pointer"
                title="BPM -1"
              >
                -
              </button>
              <input
                ...
                className="w-8 sm:w-10 bg-transparent text-center font-bold text-xs focus:outline-none"
                style={{ color: 'var(--theme-accent, #38bdf8)' }}
              />
              <button
                type="button"
          ...
          <div className="flex items-center gap-1 sm:gap-1.5">
            ...
            <div className="flex items-center gap-0.5 bg-[#0e1520]/90 border border-[#1c2738] px-1 py-0.5 rounded">
              ...
              <input
                ...
                className="w-6 sm:w-7 bg-transparent text-center font-bold text-xs text-[#e2e8f0] focus:outline-none"
              />
              <button
                type="button"
          ...
          <div className="flex items-center gap-1 sm:gap-1.5">
            ...
            <div className="flex items-center gap-0.5 sm:gap-1 bg-[#0e1520]/90 border border-[#1c2738] px-1 sm:px-1.5 py-0.5 rounded">
              <button
                  ...
                  {allowedChannels.map((count) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-1 sm:px-1.5 py-0.5 text-[10px] sm:text-[10.5px] font-bold rounded-[3px] transition-all cursor-pointer ${
                          isActive 
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
              ...
              {Array.from({ 
                ...
                return (
                  <button
                    ...
                    className={`h-5 px-1.5 sm:px-2 text-[10px] font-bold rounded-[2px] transition-colors cursor-pointer ${
                      isBankActive
          ...
          <div className="inline-flex items-center rounded overflow-hidden border border-white/10 bg-[#090e15]">
            <button
              ...
              className="h-6 px-2 xl:px-2.5 text-[11px] font-medium flex items-center gap-1 cursor-pointer border-r border-white/10 text-[#cbd5e1] hover:text-white hover:bg-white/5 transition-colors"
            ...
            <button
              ...
              className={`h-6 px-2 xl:px-2.5 text-[11px] font-medium flex items-center gap-1 cursor-pointer border-r border-white/10 transition-colors ${
            ...
            <button
              ...
              className="h-6 px-2 xl:px-2.5 text-[11px] font-medium flex items-center gap-1 cursor-pointer text-[#cbd5e1] hover:text-[#fb7185] hover:bg-rose-500/10 transition-colors"
      ...
      <div className="flex-1 min-h-0 overflow-hidden flex flex-row relative">
        ...
        >
          ...
          <div 
            ...
            style={{ width: '100%', minWidth: `${channelsCount * getTrackWidth(channelsCount)}px` }}
          >

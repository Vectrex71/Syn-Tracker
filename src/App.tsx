...
function audioBufferToWavArrayBuffer(audioBuffer: AudioBuffer): ArrayBuffer {
  ...
  const handleImportSid = async (file: File) => {
    try {
      ...
      setSidInfoData({
        ...
        chipName: 'MOS 6581/8580 SID',
        channelsCount: detailed.song.channelsCount,
        fileName: file.name,
  ...
  return (
    ...
    >
      ...
      {/* Tablet Portrait Mode Reminder Overlay */}
      {!showLanding && !isMobileDevice && (
        <TabletOrientationOverlay />
      )}
      ...
      <motion.div 
        ...
        className="absolute inset-0 pointer-events-none z-0 bg-cover bg-center bg-no-repeat mix-blend-luminosity filter brightness-95 contrast-105 origin-center will-change-transform"
        style={{ 
          backgroundImage: `url('/Studiopaper.jpeg')`,
          transform: 'scale(var(--landing-bg-zoom, 1))',
          transition: 'transform 0.28s cubic-bezier(0.2, 0, 1)',
        }}
      />
      ...
      <main className="flex-1 p-2 sm:p-2.5 flex flex-col lg:flex-row gap-2 sm:gap-2.5 overflow-hidden min-h-0 relative z-10">
        ...
        <motion.div 
          ...
          className="w-full lg:w-40 xl:w-48 2xl:w-52 shrink-0 flex flex-col h-full min-h-0 overflow-hidden"
        >
          <OrderList
            orderList={song.orderList}
            currentOrderIndex={currentOrderIndex}
        ...
        <motion.div 
          ...
          className="w-full lg:w-64 xl:w-80 2xl:w-88 shrink-0 flex flex-col h-full min-h-0 overflow-hidden"
        >
          <SampleManager
            samples={song.samples}
            selectedSampleIndex={selectedSampleIndex}

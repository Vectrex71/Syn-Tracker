...
export const SynEditorWaveform: React.FC<SynEditorWaveformProps> = ({
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-[#070c13] rounded-xl border border-[#1e2d42] shadow-inner p-3 gap-2.5">
      {/* Sample Slot Selector, Name & Apply Bar (Positioned directly above DURATION) */}
      {samples && activeSlot !== undefined && setActiveSlot && (
        <div className="flex flex-wrap items-center justify-between gap-2 bg-[#090f18] px-2.5 py-1.5 rounded-lg border border-[#1b283b] shrink-0">
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1.5 bg-[#101824] px-2 py-0.5 rounded border border-[#223348] text-xs">
            ...
            <input
              ...
              className="bg-[#101824] border border-[#223348] text-xs font-mono font-bold text-white px-2.5 py-0.5 rounded focus:outline-none focus:border-sky-500 w-28 sm:w-40"
              maxLength={24}
            />

            {onApplyToSlot && (
              <button
                onClick={onApplyToSlot}
                className="aqua-gloss aqua-green h-7 px-3 rounded text-xs font-bold flex items-center gap-1.5 cursor-pointer shadow-sm active:scale-95"
          ...
          <div className="hidden sm:flex items-center gap-2 text-[11px] font-mono text-slate-400">
            <span className="text-slate-500 text-[10px]">CURRENT TARGET:</span>
            <span className="px-1.5 py-0.5 rounded bg-sky-950/70 border border-sky-500/30 text-sky-300 font-bold text-[10.5px]">
              Slot #{activeSlot.toString().padStart(2, '0')}
            </span>
            <span className="truncate max-w-[120px] text-slate-300">
              {samples[activeSlot]?.name || 'Empty'}
            </span>
          </div>
        </div>
      )}

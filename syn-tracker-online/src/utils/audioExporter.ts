 ...
 */
function buildSongTimeline(song: TrackerSong, loopsCount = 1): ScheduledTimelineRow[] {
  const timeline: ScheduledTimelineRow[] = [];
  const baseOrderList = song.orderList && song.orderList.length > 0 ? song.orderList : [0];
  const safeLoops = Math.max(1, Math.min(64, Math.floor(loopsCount || 1)));
  const orderList: number[] = [];
  for (let l = 0; l < safeLoops; l++) {
    orderList.push(...baseOrderList);
  }
...
export async function renderSongToAudioBuffer(
  ...
  isCancelled?: () => boolean,
  options?: { loopsCount?: number; sampleRate?: number }
): Promise<AudioBuffer> {
  ...
  const timeline = buildSongTimeline(song, loopsCount);
  if (isCancelled?.()) throw new Error('Render cancelled');

...
export async function testUmamiConnection(
  ...
  if (!websiteId) {
    return {
      success: false,
      status: 0,
      message: 'Website ID missing',
      details: 'Please enter a valid Umami Website ID (UUID format).'
    };
  }
  ...
  try {
    ...
    if (res.ok) {
      return {
        ...
        details: `Umami server received the test event (HTTP ${res.status} OK). Tracking is active.`
      };
    }
    ...
    if (res.status === 404) {
      return {
        ...
        details: `Website ID "${websiteId}" was not found on Umami server "${hostUrl}". Please check the ID in your Umami dashboard under Settings > Websites.`
      };
    }
    ...
    if (res.status === 400) {
      return {
        ...
        details: errorBody || 'Payload format or Website ID was rejected by Umami server.'
      };
    }
    ...
    return {
      success: false,
      status: res.status,
      message: `Server response HTTP ${res.status}`,
      details: errorBody || `Umami server responded with status code ${res.status}.`
    };
  } catch (err: any) {
    return {
      success: false,
      status: 0,
      message: 'Connection blocked or unreachable',
      details: err?.message || 'Check your internet connection or if an adblocker (e.g. uBlock, Brave Shields) is blocking tracking requests.'
    };
  }
}
